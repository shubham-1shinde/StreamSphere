...
const MyVideoCard = ({_id, title, views, timeAgo, username, duration, thumbnail, isPublished }) => {
  ...
  return (
    <div className="bg-black rounded overflow-hidden text-white w-full h-full flex flex-col">
        <button className=" rounded overflow-hidden text-white relative bg-gray-800 h-40 w-full flex items-center justify-center cursor-pointer" onClick={() => {
            navigate(`/videos/${_id}`)
        }}>
      ...
      <div className="flex w-full justify-between pl-4 pr-4 items-center" >
        ...
        <div>
            ...
            onClick={async () => {
                ...
                await axios.patch(`/v1/videos/toggle/publish/${_id}`).then((response) => {
